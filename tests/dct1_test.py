...
class TestDct1(unittest.TestCase):
    ...
    def test_encode_dct1(self):
        ...
        self.assertTrue(np.array_equal(np.rint(encoded.getDadosComprimidos()).astype(int), [15, -4, 0, -1, 15, -4, 0, -1]))

    def test_encode_com_tamanho_nao_divisivel_dct1(self):
        encoded = codec.encode(np.array([1, 2, 3, 4, 1, 2]), 2, 2, dct1)
        self.assertTrue(np.array_equal(np.rint(encoded.getDadosComprimidos()).astype(int), [15, -4, 0, -1, 5,  3, -1, -3]))
